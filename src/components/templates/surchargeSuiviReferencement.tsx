...
/**
 * <b>Composant qui permet d'afficher le suivi de référencement</b>
 */
export const SurchargeSuiviReferencement = ({ model, modelFields }: checkReferencingProps) => {
  ...
  // TODO !dynamise le champ numero_demande ou mettre en paramètre
  const numero_demande = (record && record["numero_demande"]) || "";

  return (
    <div className="flex items-center justify-center flex-col h-screen w-full bg-[var(--color-container-bg)] p-6">
      <div className="w-full mx-auto max-w-4xl p-4 lg:p-10 bg-[var(--background-color-modal)] rounded-md shadow-md overflow-scroll">
        ...
        <div>
          ...
          <div className="grid grid-rows-3 grid-flow-col h-[150px] p-2 ">
            {modelFields && Array.from(modelFields.values()).map((field) => {
              return (
                field.viewable && (
                  <div className="" key={field.code}>
                    <DisplayField
                      fieldMetadata={field}
...
templatesDictionary.registerTemplate('surcharge_suivi_referencement', SurchargeSuiviReferencement);
