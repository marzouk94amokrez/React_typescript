...
validationRules.register("max", (
  ...
  if (['integer', 'decimal'].includes(fieldMetadata.type || '')) {
    if (length < parseFloat(value)) {
      return sprintf(t("validation.error.max"), { field: fieldLabel, value: length })
    }
  } else if (length < value.length) {
    return sprintf(t("validation.error.maxlength"), { field: fieldLabel, length })
  }
